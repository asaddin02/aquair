  ...
  return (
    <>
      ...
      <div className="k-body">
        ...
        {rit && <section className="cargo-dashboard" aria-label="Ringkasan muatan"><div className="cargo-header"><span><Ikon n="rit" s={18} />Muatan hari ini</span><span className="cargo-status"><span className="status-dot" />{aktif ? 'Rit aktif' : 'Rit selesai'}</span></div><div className="cargo-center"><div><span>Sisa di motor</span><strong className="num">{sisa}<small>galon isi</small></strong></div><div className="cargo-ring" style={{ '--progress': `${persen}%` }}><div><b className="num">{terjual}</b><span>terjual</span></div></div></div><div className="cargo-bottom"><span>Dari <b>{rit.dibawa} galon</b> yang dibawa</span><span><Ikon n={rit.muatan_dicek ? 'cek' : 'awas'} s={15} />{rit.muatan_dicek ? 'Dicek bos' : 'Belum dicek bos'}</span></div></section>}
